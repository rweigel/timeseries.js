...
plotjs.annotation.set = function (id, cb) {
  console.log(plotjs.figure[id].layout)
  ...
  let [labelAnnotations, tickShapes] = axisTicksAndLabels(layout)

  plotjs.log(`${id} creating arrow`,'annotation')
  const arrow_ = arrow(id)
  const originAnnotation = originSymbol(layout)
  let xticklen = _fullLayout.xaxis.ticklen
  let ticks = _fullLayout.xaxis.ticks
  let showticklabels = _fullLayout.xaxis.showticklabels
  ticks = ''
  ...
  const layoutNew = {
    ...
    "xaxis.title.text": "",
    "xaxis.ticks": '',
    "xaxis.showticklabels": showticklabels,
    "xaxis.minor.ticks": '',
  }
  ...
  function arrow(id) {
    const layout = plotjs.figure[id].layout
  ...
  function axisTicksAndLabels (layout, axis) {
    if (!axis) {
      const [annotationsx, shapesx] = axisTicksAndLabels(layout, 'xaxis')
      const [annotationsy, shapesy] = axisTicksAndLabels(layout, 'xaxis')
    ...
    width = xaxis.c2p(xaxis.range[1]) - xaxis.c2p(xaxis.range[0])
    height = yaxis.c2p(yaxis.range[0]) - yaxis.c2p(yaxis.range[1])
    plotjs.log(`${id}/${axis} width = ${width}, height = ${height}`, 'axisTicksAndLabels')
    ...
    for (let i = 0; i < minorTickValues.length; i++) {
      console.log(`${id}/${axis} minor tick at ${minorTickValues[i]}`)
      let deltap = ticklen/2 // Tick length for on positive side
      let deltan = ticklen/2 // Tick length for on negative side
      if (_ticks === 'outside') {
        deltap = 0
