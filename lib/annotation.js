...
plotjs.annotation.set = function (id, x, y, layout, config) {
  ...
  function axisAnnotations (id) {
    ...
    if (xlabel.font === undefined) {
      xlabel.font = domElement._fullLayout.xaxis.tickfont
    }
    ...
    if (ylabel.font === undefined) {
      ylabel.font = domElement._fullLayout.yaxis.tickfont
    }
  ...
  function axisTicksAndLabels (id, layout, direction) {
    ...
    let xYAnchor = 'top'
    let xYbottom = bottom
    if (layout[axisName].ticklabelposition === 'inside') {
      xYbottom = bottom + c2pRatio * layout[axisName].ticklen
      xYAnchor = 'bottom'
    }
