...
plotjs.annotation.set = function (id, x, y, layout, config) {
  ...
  function axisAnnotations (id) {
    ...
    let xfont = domElement._fullLayout.xaxis.tickfont
    let yfont = domElement._fullLayout.yaxis.tickfont
    ...
    const annotations =
      [
        {
          text: '$\\Large x$',
          x: lastGrid.x + lastMinorTickStep.x / 2,
        ...
        },
        {
          ...
          x: lastGrid.x + lastMinorTickStep.x,
          y: 0,
          ax: xrange[0] < 0 && dataRange_.x[0] >= 0 ? 0 : dataRange_.x[0],
          ay: 0,
          ...layout._annotations.xarrow
        },
        {
          text: '$\\Large y$',
          x: coordinateToPixelRatio(id, 'x') * 16,
          y: lastGrid.y + lastMinorTickStep.y / 2,
          font: yfont,
          showarrow: false
        },
        {
          ...
          x: 0,
          y: lastGrid.y + lastMinorTickStep.y,
          ax: 0,
          ay: yrange[0] < 0 && dataRange_.y[0] >= 0 ? 0 : dataRange_.y[0],
          ...layout._annotations.yarrow
        }
      ]
    return annotations
  }
  ...
  function axisTicksAndLabels (id, layout, direction) {
    ...
    if (layout[axisName].ticks === '_centered') {
