...
plotjs.annotation.set = function (id, x, y, layout, config) {
  ...
  let adjustments = adjustedRange()

  const range = { x: adjustments.x.range, y: adjustments.y.range }
  ...
  Plotly.relayout(domElement.id, plotjs.util.deepCopy(layoutNew))
    .then(() => {
      ...
      return
      adjustments = adjustedRange()
      console.log(adjustments)

      layoutNew.xaxis.autorange = false
      layoutNew.yaxis.autorange = false

      layoutNew.xaxis.range = adjustments.x.range
      layoutNew.xaxis.tickmode = 'array'
      layoutNew.xaxis.tickvals = adjustments.x.majorTickVals
      layoutNew.xaxis.ticktext = adjustments.x.majorTickLabels
      layoutNew.xaxis.minor.tickmode = 'array'
      layoutNew.xaxis.minor.tickvals = adjustments.x.minorTickVals

      layoutNew.yaxis.range = adjustments.y.range
      layoutNew.yaxis.tickmode = 'array'
      layoutNew.yaxis.tickvals = adjustments.y.majorTickVals
      layoutNew.yaxis.ticktext = adjustments.y.majorTickLabels
      layoutNew.yaxis.minor.tickmode = 'array'
      layoutNew.yaxis.minor.tickvals = adjustments.y.minorTickVals

      domElement.style.display = ''

      plotjs.log(`${domElement.id} calling Plotly.relayout() to adjust range to end on grid line`, 'annotation')
      Plotly.relayout(domElement.id, plotjs.util.deepCopy(layoutNew))
        .then(() => {
          const msg = `${domElement.id} called Plotly.relayout() to adjust range to end on grid line. Showing figure.`
          plotjs.log(msg, 'annotation')
  ...
  function adjustedRange () {
    ...
    for (const dir of ['x', 'y']) {
      adjustments[dir] = {}
      const majorTickVals = majorTicks(dir)
      const majorTickLabels = majorTicks(dir, 'labels')
      const minorTickVals = minorTicks(dir)
      ...
      plotjs.log(`${domElement.id}/${dir} major ticks = ${majorTickVals}`, 'adjustedRange')
      plotjs.log(`${domElement.id}/${dir} minor ticks = ${minorTickVals}`, 'adjustedRange')
      ...
      const firstGridIsMajor = Math.min(majorTicks(dir)) < Math.min(minorTicks(dir))
      const lastGridIsMajor = Math.max(majorTicks(dir)) > Math.max(minorTicks(dir))

      const axis = _fullLayout[dir + 'axis']
      let deltaFirst = 0
      if (firstGridIsMajor) {
        deltaFirst = axis.gridwidth * coordinateToPixelRatio(dir)
        minorTickVals.unshift(allTicks_[0] - firstTickDelta)
        //majorTickLabels.unshift('')
      } else {
        deltaFirst = axis.minor.gridwidth * coordinateToPixelRatio(dir)
        minorTickVals.unshift(allTicks_[0] - firstTickDelta)
      }
      let deltaLast = 0
      if (lastGridIsMajor) {
        deltaLast = axis.gridwidth * coordinateToPixelRatio(dir)
        minorTickVals.push(allTicks_[allTicks_.length - 1] + lastTickDelta)
        //majorTickLabels.unshift('')
      } else {
        deltaLast = axis.minor.gridwidth * coordinateToPixelRatio(dir)
        minorTickVals.push(allTicks_[allTicks_.length - 1] + lastTickDelta)
      }
      // The 2 is to prevent the last grid line from being clipped. It should
      // not be needed. As a result, the horizontal grid lines will extend past the
      // first and last vertical grid lines by a pixel. Same issue for other direction.
      // I don't see a way to fix without manually creating grid lines.
      const first = allTicks_[0] - lastTickDelta - 2 * deltaFirst
      const last = allTicks_[allTicks_.length - 1] + lastTickDelta + 2 * deltaLast

      adjustments[dir].range = [first, last]
      adjustments[dir].majorTickVals = majorTickVals
      adjustments[dir].majorTickLabels = majorTickLabels
      adjustments[dir].minorTickVals = minorTickVals
      plotjs.log(`${domElement.id}/${dir} old range = ${axisRange(dir)}`, 'adjustedRange')
      plotjs.log(`${domElement.id}/${dir} new range = ${adjustments[dir].range}`, 'adjustedRange')
    }
    return adjustments
  }
